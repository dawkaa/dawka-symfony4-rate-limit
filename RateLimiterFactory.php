...
final class RateLimiterFactory
{
    ...
    protected static function configureOptions(OptionsResolver $options): void
    {
        ...
        $options->setAllowedValues('policy', ['token_bucket', 'fixed_window', 'sliding_window', 'no_limit']);
        $options->setAllowedTypes('limit', 'int');
        $options->setAllowedTypes('interval', 'string');
        $options->setNormalizer('interval', $intervalNormalizer);
        $options->setDefault('rate', function (OptionsResolver $rate) use ($intervalNormalizer) {
            $rate->setDefined(['amount']);
            $rate->setAllowedTypes('amount', 'int');
            $rate->setDefault('amount', 1);
            $rate->setDefined(['interval']);
            $rate->setAllowedTypes('interval', 'string');
            $rate->setNormalizer('interval', $intervalNormalizer);
        });
        $options->setNormalizer('rate', function (Options $options, $value) {
